...
py1 = lambda x: np.log(P2) + np.log(lm2) - np.log(2 * np.pi * np.sqrt(Dx21 * Dx22)) - (x[0] - mx21) ** 2 / (2 * Dx21) - (x[1] - mx22) ** 2 / (2 * Dx22)
py_1 = lambda x: np.log(P1) + np.log(lm1) - np.log(2 * np.pi * np.sqrt(Dx11 * Dx12)) - (x[0] - mx11) ** 2 / (2 * Dx11) - (x[1] - mx12) ** 2 / (2 * Dx12)
predict = []
for i in x_train:
    predict.append(np.argmax([py_1(i), py1(i)]) * 2 - 1)
